...
export function stepBoth(sim0: Sim, sim1: Sim) {
  sim0._netInternal.receivePacket(unwrap(sim1._netInternal.getOutboundPacket(0)));
  sim1._netInternal.receivePacket(unwrap(sim0._netInternal.getOutboundPacket(1)));
  sim0.step();
  sim1.step();
}
