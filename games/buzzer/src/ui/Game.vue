...
import { buzzer, netStatus } from '../ui';
...
const message = computed(() => {
  switch (buzzer.value.phase) {
    ...
    case 'active':
      return 'CLICK NOW!';
    case 'won':
      if (buzzer.value.winner === null) {
        return 'YOU HAVE DISCOVERED A BUG!';
      }
      if (netStatus.value.ourId === null || buzzer.value.winner === netStatus.value.ourId + 1) {
        return 'YOU WIN!';
      } else {
        return 'THEY WIN!';
      }
...
const isPlayerOne = computed(() => {
  return netStatus.value.ourId === 0 || netStatus.value.ourId === null;
})
</script>
...
<template>
  <div ref="container" class="buzzer-game" :style="{ backgroundColor }">
    ...
    <div class="scores">
      <div class="player1">
        <span class="label">{{ isPlayerOne ? "You" : "Them" }}</span>
        <span class="score">{{ buzzer.player1Score }}</span>
      </div>
      <div class="player2">
        <span class="label">{{ isPlayerOne ? "Them" : "You" }}</span>
